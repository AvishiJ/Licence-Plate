...
def getOCR(im, coors):
    ...
    if ocr != "":  # Only add non-empty OCR results
        # Read existing plates to check for duplicates
        try:
            with open("plates.txt", "r") as f:
                existing_plates = set(line.strip() for line in f)  # Load all plates into a set
        except FileNotFoundError:
            existing_plates = set()  # If the file doesn't exist, create an empty set

        # Add the new plate only if it's not already in the file
        if ocr not in existing_plates:
            with open("plates.txt", "a") as f:  # Append new plate to the file
                f.write(ocr + "\n")  # Write the detected plate with a newline
